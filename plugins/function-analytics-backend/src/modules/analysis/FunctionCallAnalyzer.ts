...
export function analyzeFunctionCalls(rawTraces: any[]): FunctionAnalysis[] {
  ...
  rawTraces.forEach(trace => {
    ...
    trace.spans.forEach((span: any) => {
      ...
      // Ignore direct hits generated by the blind orchestrator test loop.
      // This allows the analyzer to focus exclusively on authentic internal service-to-service
      // workflow boundaries and actual traffic-generator bots.
      if (callerService.includes('backstage-backend')) {
        return;
      }

      // Map automated test drivers to external clients instead of treating them as microservice peers.
      if (
        callerService.includes('backstage-backend') ||
