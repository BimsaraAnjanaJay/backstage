...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [codeAnalysisBackendPlugin],
    });

    await request(server).get('/api/code-analysis-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/code-analysis-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/code-analysis-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/code-analysis-backend/todos/${createdTodoItem.id}`)
